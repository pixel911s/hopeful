...
async function closeTask(conn, id, username) {
    try {
        // ปิด Task
        let sql = "update task set `isClose`=true, `closeDate`=? `updateBy`=?, `updateDate`=? where id = ?";
...
async function recallTask(conn, id, username) {
    try {
        ...
        let sql = "update task set `isClose`=false, `closeDate`=? `updateBy`=?, `updateDate`=? where id = ?";
