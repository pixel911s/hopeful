...
async function clearOwner(conn, agentId, date) {
  try {
    //Update Activity Status
    let sql =
      "update activity set `ownerUser`= null, `updateBy`= 'SYSTEM', `updateDate`= NOW() where NOW() > dueDate + INTERVAL ? DAY and agentId = ?";

    await conn.query(sql, [date, agentId]);
